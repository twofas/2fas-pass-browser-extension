...
const bgConnectOnClose = async (event, data) => {
  ...
  // Once the connect socket closes (success, error, timeout or cancel) the in-flight
  // session is over, so drop the persisted resume blob + keepalive alarm + self-tick.
  // The resume path is ONLY for a Safari SW kill, where no close event fires at all.
