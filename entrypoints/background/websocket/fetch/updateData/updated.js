...
const updateDataUpdated = async (info, state, hkdfSaltAB, sessionKeyForHKDF, messageId) => {
  ...
  try {
    ...
    if (item.securityType === SECURITY_TIER.SECRET) {
      ...
      const encryptionItemT3KeyAESRaw = await crypto.subtle.exportKey('raw', encryptionItemT3Key);
      ...
      const encryptionItemT2KeyAESRaw = await crypto.subtle.exportKey('raw', encryptionItemT2Key);
    ...
    return {
      ...
      returnToast: {
        text: getMessage('fetch_update_login_updated_toast'),
        type: 'success'
      }
    };
  } catch (e) {
