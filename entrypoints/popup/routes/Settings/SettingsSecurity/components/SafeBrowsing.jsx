// SPDX-License-Identifier: BUSL-1.1
//
// Copyright © 2025 Two Factor Authentication Service, Inc.
// Licensed under the Business Source License 1.1
// See LICENSE file for full terms

import S from '../../Settings.module.scss';
import bS from '@/partials/global-styles/buttons.module.scss';
import { useEffect, useState } from 'react';

/**
* Function to render the Safe Browsing component.
* @return {JSX.Element} The rendered component.
*/
function SafeBrowsing () {
  const [loading, setLoading] = useState(true);
  const [sB, setSB] = useState(null);
  const [disabled, setDisabled] = useState(true);

  useEffect(() => {
    const getDefaultSafeBrowsing = async () => {
      const browserSafeBrowsing = await browser.privacy.services.safeBrowsingEnabled.get({});
      setSB(browserSafeBrowsing);
      setLoading(false);
      setDisabled(false);
    };

    try {
      getDefaultSafeBrowsing();
    } catch (e) {
      CatchError(e);
    }
  }, []);

  const handleChange = async () => {
    setDisabled(true);

    try {
      await browser.privacy.services.safeBrowsingEnabled.set({ value: !sB });
      setSB(!sB);
      showToast(browser.i18n.getMessage('notification_safe_browsing_success'), 'success');
    } catch (e) {
      showToast(browser.i18n.getMessage('error_safe_browsing_saving'), 'error');
      await CatchError(e);
    } finally {
      setDisabled(false);
    }
  };

  if (loading) {
    return null;
  }

  return (
    <div className={S.settingsSafeBrowsingReports}>
      <form action="#" className={bS.passToggle}>
        <input type="checkbox" name="pass-safe-browsing" id="pass-safe-browsing" defaultChecked={sB} onChange={handleChange} disabled={disabled ? 'disabled' : ''} />
        <label htmlFor="pass-safe-browsing">
          <span className={bS.passToggleBox}>
            <span className={bS.passToggleBoxCircle}></span>
          </span>
  
          <span className={bS.passToggleText}>
            <span>{browser.i18n.getMessage('settings_safe_browsing')}</span>
          </span>
        </label>
      </form>
    </div>
  );
}

export default SafeBrowsing;
